...
IplImage *poisson_blend(IplImage *I, IplImage *mask, int posx, int posy)
{
	...
	for(int i=posx, ii =0;i<posx+h1;i++,ii++)
		for(int j=0,jj=posy;j<w1;j++,jj++)
			for(int c=0;c<channel;++c)
			{
				if(abs(CV_IMAGE_ELEM(sgx,float,ii,j*channel+c) - CV_IMAGE_ELEM(sgy,float,ii,j*channel+c)) >
                   abs(CV_IMAGE_ELEM(grx,float,ii,j*channel+c) - CV_IMAGE_ELEM(gry,float,i,j*channel+c)))
				{
				...
				{
					CV_IMAGE_ELEM(bmaskx,float,i,jj*channel+c) = CV_IMAGE_ELEM(grx,float,i,jj*channel+c);
					CV_IMAGE_ELEM(bmasky,float,i,jj*channel+c) = CV_IMAGE_ELEM(gry,float,i,jj*channel+c);
				}
			}
